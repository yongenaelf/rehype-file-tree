...
describe("validation", () => {
  test("throws an error with no content", () => {
    expect(() => processTestFileTree("")).toThrowErrorMatchingInlineSnapshot(
      `[Error: The \`<FileTree>\` component expects its content to be a single unordered list but found no child elements.]`
    );
  });
  ...
  test("throws an error with multiple root elements", () => {
    expect(() =>
      ...
      `[Error: The \`<FileTree>\` component expects its content to be a single unordered list but found multiple child elements: \`<p>\` - \`<ul>\`.]`
    );
  });

  test("throws an error with no root ordered list", () => {
    expect(() =>
      processTestFileTree("<ol><li>file</li></ol>")
    ).toThrowErrorMatchingInlineSnapshot(
      `[Error: The \`<FileTree>\` component expects its content to be an unordered list but found the following element: \`<ol>\`.]`
    );
  });

  test("throws an error with no list item", () => {
    expect(() =>
      processTestFileTree("<ul></ul>")
    ).toThrowErrorMatchingInlineSnapshot(
      `[Error: The \`<FileTree>\` component expects its content to be an unordered list with at least one list item.]`
    );
  });
});
...
describe("processor", () => {
  ...
  <li>
    ...
    expect(extractFileTree(html)).toMatchFileSnapshot(
      "./snapshots/file-tree-basic.html"
    );
  });
  ...
  test("processes text comments following the file name", () => {
    const html = processTestFileTree(
      `<ul><li>file this is a comment</li></ul>`
    );
  ...
  <li>another_directory
    ...
    expect(extractFileTree(html).match(/<li class="directory">/g)).toHaveLength(
      2
    );
  });
  ...
  test("identifies placeholder with either 3 dots or an ellipsis", () => {
    ...
    expect(
      extractFileTree(html).match(/<li class="[\w\s]*empty[\w\s]*">/g)
    ).toHaveLength(2);
  });
...
describe("icons", () => {
  test("adds a folder icon to directories with a screen-reader only label", () => {
    ...
    expect(extractFileTree(html)).toContain(
...
function expectHtmlToIncludeIcon(
  html: string,
  icon: (typeof Icons)[keyof typeof Icons]
) {
  return expect(extractFileTree(html, false)).toContain(
    icon.replace("/>", ">")
  );
}
